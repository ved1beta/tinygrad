...
add_buffer_ops = PatternMatcher([
  ...
  # swizzle the STORE ShapeTracker last
  (UPat(Ops.STORE, src=(UPat(), UPat(), UPat(Ops.VIEW, src=(UPat.var("base"),))), name="store"),
   lambda base,store: store.replace(src=(store.src[0], (store.src[1].st+base.st).to_uop(), base))),
...
DONT_PUSH_VIEWS = {Ops.BUFFER, *GroupOp.Buffer, Ops.ASSIGN, Ops.DEVICE, Ops.CONST, Ops.SINK}
...
def swizzle_reduceop(r:UOp, src:UOp, view:UOp):
  ...
  # swizzle the input and create new axis
  new_axis = tuple(range(len(st.shape), len(st.shape) + len(r.axis_arg)))
  new_src = apply_swizzle(UOp(Ops.VIEW, src.dtype, (src,), tmp+ShapeTracker(tuple(nv))))
...
def elementwise_view_right(root:UOp) -> UOp|None:
  ...
  ret = root.replace(src=tuple(x.base if x.base.shape == new_st.shape else apply_swizzle(UOp(Ops.VIEW, x.dtype, (x,), new_st)) for x in root.src))
